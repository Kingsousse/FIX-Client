...
public class ReplyRejectionConnection extends RejectionMessage {

	@Override
	public  Message deserialize(String[] items) throws InvalidMessageException {
		
		if(items[1].split("=").length == 2 && Integer.valueOf(items[1].split("=")[0]) == DictionaryTag.REASON.getValue()  ) {
	...
	public MsgProtocoleType getProtocoleType() {
		return MsgProtocoleType.LOGON_REJECT;
	}
